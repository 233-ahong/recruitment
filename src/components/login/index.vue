      ...
      <div v-else>
        ...
        <el-form-item prop="code">
          ...
          <div class="login-code">
            <el-button style='float: right' :disabled="codeOff" @click='getCode' class='login-code-img'>{{codeString}}</el-button>
          </div>
        </el-form-item>
      </div>
      <el-form-item style="width: 100%">
        ...
        <div style="width: 350px">
          <el-row>
            ...
            <el-col :span='12'/>
            <el-col :span='6'>
              <el-link @click='editLoginType' class='login-type'>{{loginType}}</el-link>
            </el-col>
          </el-row>
        </div>
      </el-form-item>
...
const loginRules = {
  ...
  phone: [{ pattern: /^1[3-9]\d{9}$/ ,trigger: 'blur', message: '请输入正确的电话号码' },
  ...
  proxy.$refs.loginRef.validate((valid) => {
    if (valid) {
      ...
      if (isPassword.value) {
        ...
        loginByPhone(loginForm.value).then(res => {
          setToken(res.data.access_token)
          router.push({ path: redirect.value || '/' })
        }).catch(() => {
          loading.value = false
